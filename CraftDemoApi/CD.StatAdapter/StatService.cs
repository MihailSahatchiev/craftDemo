...
namespace CD.StatAdapter
{
	public class StatService : IStatService
	{
		...
		public async Task<AllCountriesPopulation> GetCountryPopulationsAsync(AllCountriesPopulation model)
		{
			var listOfCountries = GetCountryPopulations();
			foreach(var country in listOfCountries)
			{
				if (!model.AllCountriesPopulationList.ContainsKey(country.Item1))
				{
					model.AllCountriesPopulationList.Add(country.Item1, country.Item2);
				}
			}
			return model;
		}
